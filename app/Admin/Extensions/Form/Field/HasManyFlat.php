...
class HasManyFlat extends Field
{
    /**
     * Relation name.
     *
    ...
    protected $tableReplacement = 'new___PARENT_KEY__';
     ...
     */
    protected $views = [
        'default' => 'admin.form.hasmany_flat',
        'table' => 'admin.form.hasmanytable_flat'
    ];
    ...
    protected $template = 'admin.form.input_hasmany';
...
//        $this->parent = $parent;
     ...
     *
     * @return \Encore\Admin\Form\Field\HasMany
     *
     * @author Edwin Hui
     */
...
$(document).on('click', '.{$this->column}-parent-form-add', function () {

//var index = $(document).find('.has-many-{$this->column}-form').length;



var tRoot = $(this).parents('.has-many-{$this->column}');

    var tpl = $(tRoot).find('template.{$this->column}-tpl:first');

    index++;
    
    var template = tpl.html().replace(/__PARENT_KEY__/g, index).replace(/__LA_KEY__/g, index);
    $(tRoot).find('.has-many-{$this->column}-forms').append(template);
    {$templateScript}
...
$(document).on('click', '.remove.btn-danger', function () { //.{$this->column}-parent-form-remove
    $(this).parents('.has-many-{$this->column}-form').hide();
    $(this).parents('.has-many-{$this->column}-form').find('.$removeClass').val(1);
});
...
//var index = 0;
$(document).on('click', '.{$this->column}-form-add-table', function () {
// tpl ROOT
    var tableRoot = $(this).parents('table');
    console.log('tableRoot',tableRoot);
    var parentTpl = $(document).find('template.{$this->parent}-tpl:first').html();
    var tpl = $(parentTpl).find('template.{$this->column}-tpl:first');
// parent NEST NR
    var rootNr = $(this).parents('.has-many-{$this->parent}-form').attr('rel');
// table TRs NR
    var tbody = $(this).parents('.has-many-table-{$this->column}').find('tbody');
    var indexRoot = $(tbody).find('tr').length + 1;

console.log('indexRoot',indexRoot);
console.log('{$defaultKey}','{$this->tableReplacement}');
...
//    index++;
    ...
    var template = tpl.html().replace(/new___PARENT_KEY__/g, rootNr)
        .replace(/__LA_KEY__/g, indexRoot++)
        .replace(/new_new/g,'new');
...
$(document).on('click', '.remove.btn-warning', function () { //{$this->column}-form-remove-table

console.log('kkjkj');

    $(this).parents('.has-many-table-{$this->column}-form').hide();
    $(this).parents('.has-many-table-{$this->column}-form').find('.$removeClass').val(1);
});
EOT;
